...
Актуально на 2026-08-05. Этот документ не содержит паролей, токенов и закрытых
...
7. Запустить один контролируемый webhook-прогон и проверить:
   ...
   - FOC имеет `Total,$ = 0`;
   - строки компонентов набора остаются без `#`, но содержат собственные
     денежные поля из invoice;
   - при разбивке компонента по паллетам сумма total до/после скидки и
     commercial discount равна его исходной строке invoice, а packing-поля
     остаются пустыми;
...
### Обновление color SKU в `_master_catalog`

Справочник поддерживает одновременный поиск одной карточки по новому и старым
артикулам. Контракт колонок:

- `SKU Code - 2` — текущий новый SKU;
- `SKU Code - 1` — существующий старый код карточки;
- `Old SKU FNO` — старый SKU ровно из таблицы соответствий поставщика;
- `Legacy SKU Code - 2` — значение второй SKU-колонки до миграции.

Порядок безопасного обновления:

1. Получить values-снимок production-листа и сохранить его вне Git.
2. Сформировать candidate и JSON-отчёт через
   `scripts/prepare_moroccanoil_color_catalog.py`.
3. Проверить отчёт, diff значений и прогнать builder на реальных новом и
   архивном комплектах.
4. Сначала развернуть alias-aware `workflows/build_moroccanoil.js`; он
   совместим со старой структурой справочника.
5. Запустить `scripts/apply_google_sheet_values.py` в dry-run. Затем применить
   тот же payload с `--apply --backup-title <unique-title>`. Скрипт перед
   записью сверяет production со снимком, копирует исходный лист, проверяет
   каждую записанную ячейку и откатывает значения при несовпадении.
6. Повторно прочитать `_master_catalog` из Google Sheets и прогнать реальный
   bundle уже против live-значений.

Не заполнять предположениями ТН ВЭД, перевод и упаковку для новых карточек.
Если эти поля отсутствуют в таблице поставщика, warning должен сохраниться до
получения достоверных данных.

### Статус обновления color SKU 2026-08-03

- Источник поставщика содержит 216 однозначных пар старого/нового SKU: 191
  карточка обновлена, 25 новых добавлены. Production-лист теперь содержит 237
  строк с заголовком и 644 уникальных SKU-алиаса без cross-row конфликтов.
- Backup n8n SQLite:
  `/opt/n8n/.n8n/database.sqlite.backup-moroc-color-sku-20260803T155219Z`.
- Backup Google sheet:
  `_BACKUP_master_catalog_20260803_before_color_sku_sync`, `sheetId=560740942`,
  `hidden=true`. Рабочий `_master_catalog` сохранил исходный
  `sheetId=792686501`.
- SHA-256 ноды `Build Moroccanoil Customs and CZ Rows`:
  `2288eb3c8cf90fe6e12d5b0fe34a67b73b0c58ed837633163c917c176a5df055`;
  `workflow_entity` совпадает с текущей `workflow_history`.
- Production values-read полностью совпал с candidate. На live-данных
  `ILSO000003204`: 103 customs / 105 ЧЗ, missing article/GTIN/ТН ВЭД = 0,
  warning rows = 0.
- Архивные `8251014` и `7260185` читаются по прежним SKU. Их оставшиеся
  warning-позиции существовали до color-миграции и относятся к отдельным
  наборам/аксессуарам.
- `PRAGMA integrity_check = ok`; `n8n` и `tg-upload-bot` активны; `/healthz`:
  `{"status":"ok"}`. Локально пройдено 59 тестов, 25 Node-тестов пропущены;
  Node.js syntax/real-data smoke выполнены на production-сервере.
- Независимый reviewer не нашёл critical/high замечаний. Medium-риск формул
  устранён: apply-скрипт читает `valueRenderOption=FORMULA` и прекращает
  обновление до создания backup, если меняемая ячейка содержит формулу.
...
- Строки состава набора выводятся без `#`, со своими ценами и скидками из
  invoice. В ЧЗ total-поля пропорционально делятся между pallet/batch-блоками,
  unit price и discount сохраняются, packing-поля остаются пустыми. В
  контрольных итогах component-строки не суммируются повторно с parent.
...
### Статус релиза 2026-08-05

- Собственные денежные поля компонентов набора развёрнуты в production MOIL
  build-ноде. SHA-256 установленного source:
  `9ce52890f9f13659e0aa0fa884244594f05dcf4789fb24bb269cca2ca0df38f5`.
- Backup n8n SQLite:
  `/opt/n8n/.n8n/database.sqlite.backup-moil-kit-money-20260805T211744Z`.
- Post-deploy readback подтвердил совпадение `workflow_entity` и текущей
  `workflow_history`, `PRAGMA integrity_check = ok`, активные `n8n` и
  `tg-upload-bot`, `/healthz = {"status":"ok"}`.
- Из установленного source прошли все `22` Node.js-регрессии. Реальный
  `LOAD0006732` повторно обработан без записи в Sheets: 7 invoice, 1 packing,
  1 batch; собственные цены всех пяти компонентов `126022812` совпали с
  invoice, суммы трёх ЧЗ-блоков сошлись с исходными totals.
- Reviewer выявил и до релиза помог закрыть смешивание regular batch
  standalone-товара с одноимённой component-строкой; повторное ревью — `GO`.
- Webhook не вызывался, поэтому существующие листы не пересчитывались и
  Telegram-сообщения не отправлялись. Для применения к старым листам требуется
  повторная загрузка комплекта.
