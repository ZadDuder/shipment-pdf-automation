...
7. Запустить один контролируемый webhook-прогон и проверить:
   ...
   - созданы отдельные листы `<invoiceNo>` и один `ЧЗ <shipment>`;
...
## Текущий контракт MOIL (уточнён 2026-07-27)

- Один запуск содержит все invoice одной поставки, ровно один общий packing и
  не более одного общего batch-файла.
- Результат: отдельный customs-лист `<invoiceNo>` на каждый invoice и один
  общий лист `ЧЗ <shipmentKey>`.
...
- Контракт `1 invoice + 1 packing`, введённый 2026-07-24, отменён после
  уточнения клиента и получения полного пакета 2026-07-27.
