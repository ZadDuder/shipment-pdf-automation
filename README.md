...
- `workflows/*.js` — поддерживаемые исходники Code-нод для n8n.
...
Для MOROCCANOIL строки состава набора определяются по отсутствующему номеру
позиции в invoice. В таможне они идут сразу после родительского набора. В ЧЗ
полный состав повторяется после каждой pallet/batch-строки родителя, а
количество масштабируется пропорционально количеству родительского блока.
У компонентов пустые `#`, денежные, packing- и batch-поля, чтобы расшифровка
не дублировала стоимость и логистику самого набора.

Одинаковый SKU в нескольких invoice не объединяется между документами.
Соответствующие packing/batch-строки выбираются сначала по номеру исходного
файла, затем по точному quantity и потребляются только один раз. Если
родитель разбит на несколько batch, его коробки и вес распределяются между
строками пропорционально batch quantity без умножения итогов.
