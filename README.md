...
invoice и с количеством этой паллеты. Компоненты получают собственные цены,
скидку и суммы из своих строк invoice. При разбивке по блокам unit price и
процент скидки сохраняются, а total до/после скидки и commercial discount
распределяются пропорционально количеству; их сумма сходится с исходной
component-строкой. Packing-поля компонентов остаются пустыми, а при
контрольной сверке стоимости они не прибавляются к родительской строке набора.
